...
md <- read_csv(master_data_filename, col_types = list(DUNS.. = col_character(), Control. = col_character(), Project.No. = col_character(), Proj.ZIP = col_character(),
...
shiny_app_data <- read_csv(shiny_data_filename, col_types = list(DUNS = col_character(), 
                                                                  Control.No. = col_character(), Project.No. = col_character(), Proj.ZIP = col_character(),
...
recode <- data.frame(read.csv("re-geocode_errors.csv", stringsAsFactors = FALSE))
glimpse(recode)
...
shiny_app_data %>% filter(is.na(app_lat)) %>% select(app_address, app_lat, app_lon) %>% data.frame(.) %>% head(., na_length)
...
# replace NA values of proj_address due to Proj.ZIP being NA
for(i in 1:nrow(need_proj_address)) {
        if(is.na(need_proj_address$Proj.ZIP)[i]) {
                need_proj_address$proj_address[i] <- str_c(need_proj_address$Proj.City.Name[i], need_proj_address$Proj.ST.Abbr[i], sep = ", ")
        }
}
...
# correct any known geocode errors - some are bing map errors, others are ugly addresses
setwd("G:/PNP/Performance Measurement/rshinyapp/clean_shiny_data")
recode <- data.frame(read.csv("re-geocode_proj_errors.csv", stringsAsFactors = FALSE))

# loop through list re-geocoding
for(i in 1:nrow(recode)) {
        error_address <- recode$error_address[i]
        error_row <- which(need_proj_address$proj_address == error_address)
        if(length(error_row) > 0) {
                print(error_row)
        }
        need_proj_address$proj_lat[error_row] <- recode$proj_lat[i]
        need_proj_address$proj_lon[error_row] <- recode$proj_lon[i]
}
...
# re-combine the existing master_data with the newly mapped applications
shiny_app_data_minus <- filter(shiny_app_data, !is.na(proj_address))
shiny_app_data <- rbind(shiny_app_data_minus, need_proj_address)

# check to make sure all shiny_app_data records have app_address, app_lat, app_lon
length(which(is.na(shiny_app_data$proj_address)))
shiny_app_data %>% filter(is.na(proj_address)) %>% select(FY, Project.No., Control., Status, Proj.ST.Abbr, proj_address, Proj.ZIP, Proj.City.Name)
names(shiny_app_data)
...
# dups <- shiny_jitter[dup_index, ]
# unique_dups <- unique(dups$app_coord)
# then run jitter loop (starting with for loop), and redo diagnostic jitter check above to see if duplicates successfully jittered
...
length(unique_dups)
# dups %>% select(proj_coord, proj_address) %>% data.frame(.) %>% head(., 50)
...
for(i in 1:length(unique_dups)){
        ...
        for(x in 1:nrow(shiny_jitter_dup_i)){
                ...
                if(direction_counter == 1){
                        ...
                        # print(as.numeric(shiny_jitter_dup_i$proj_lat[x]) + .01*loop_multiplier)
                        proposed_new_proj_lat <- as.numeric(shiny_jitter_dup_i$proj_lat[x]) + .01*loop_multiplier
                ...
                if(direction_counter == 3){
                        ...
                        proposed_new_proj_lon <- as.numeric(shiny_jitter_dup_i$proj_lon[x]) + .01*loop_multiplier
...
shiny_app_data <- rename(shiny_app_data, "Control.No." = Control., "EDA.Funding" = Best.EDA.., "Construction" = Cons.Non, 
       ...
       "PPS.Date" = X_PPS_Date, "PPE.Date" = X_PPE_Date, "PX1.Date" = X_PX1_Date, 
       "PX2.Date" = X_PX2_Date, "Local.Applicant.Match" = Local.Applicant.., "Coapp.DUNS" = Coapp.DUNS.., "Coapp.IRS" = Coapp.IRS.., 
       "Coapp.Appl.ZIP" = Coapp.Appl..Zip, "Est.Jobs.Created" = Jobs.Created, "Est.Jobs.Saved" = Jobs.Saved,
       "Est.Private.Investment" = Private.Investment)

# save numeric values as character, since we experienced error where excel would save numerics in scientific notation, 
# and then to read_csv properly you had to drop the col_number argument, or else it dropped all ending zeroes!?
# but since we have so many scripts already built loading data with the col_number argument, 
# it was easier to save the shiny_data numerics as characters, so excel does not use scientific notation, and read_csv with col_number arg works correctly
shiny_app_data$EDA.Funding <- as.character(shiny_app_data$EDA.Funding)
shiny_app_data$Total.Proj.Cost <- as.character(shiny_app_data$Total.Proj.Cost)
shiny_app_data$Est.Private.Investment <- as.character(shiny_app_data$Est.Private.Investment)
shiny_app_data$Local.Applicant.Match <- as.character(shiny_app_data$Local.Applicant.Match)


###################################################


# test that count/amount was unchanged
md_sum_fy <- md %>% group_by(FY) %>% summarize(count = n(), amount = sum(as.numeric(Best.EDA..), na.rm = TRUE)) %>% data.frame(.)
shiny_sum_fy <- shiny_app_data %>% group_by(FY) %>% summarize(count = n(), amount = sum(as.numeric(EDA.Funding), na.rm = TRUE)) %>% data.frame(.)
for(i in 1:nrow(md_sum_fy)) {
        count_diff <- (md_sum_fy$count[i] - shiny_sum_fy$count[i])
        amount_diff <- dollar((md_sum_fy$amount[i] - shiny_sum_fy$amount[i]))
        print(str_c("FY ", md_sum_fy$FY[i], ": count_diff: ", count_diff, "; amount_diff: ", amount_diff))
}
